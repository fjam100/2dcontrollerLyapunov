function [res, Knormal, epsilonn, epsilont]=getU2(X,Xr,model,Yprev,Ts)
...
T2=[-sin(theta) -cos(theta); cos(theta), -sin(theta)];
rn=model.spRad;
Xr2=[xc+rn*cos(theta); yc+rn*sin(theta)];
epsilonerror=dot((Xr2-[x;y]),[-cos(theta); -sin(theta)]);
Fextra=T2*[0 0; 0 100]*[0; epsilonerror];
% Fextra2=[0 ; -k*accY];
res=res+Fextra;
