...
rem(1,1)=mx.^(-1).*my.^(-1).*((x+(-1).*xc).^2+(y+(-1).*yc).^2).^(-5/2).*(cx.*my.* ...
  ...
  -4).*yc.*yrdd))))))+spk.*(x+(-1).*xc).*((-1).*xc.^2+x.*(xc+(-1).*xr)+ ...
  xc.*xr+(y+(-1).*yc).*(yc+(-1).*yr)).*Pattern(delta,Blank(r)).*(my.*( ...
  x.^2+(-2).*x.*xc+xc.^2+(y+(-1).*yc).^2).^(1/2).*((-1).*y+yc)+mx.*(x.^2+( ...
  -2).*x.*xc+xc.^2+(y+(-1).*yc).^2).*sin(atan(x+(-1).*xc,y+(-1).*yc))));
rem(2,1)=mx.^(-1).*my.^(-1).*((x+(-1).*xc).^2+(y+(-1).*yc).^2).^(-3/2).*((-1).* ...
  ...
  my.*(xd.*(y+(-1).*yc)+((-1).*x+xc).*yd).^2)+spk.*Pattern(delta,Blank(r)) ...
  .*(my.*(x+(-1).*xc).^2.*(x.^2+(-2).*x.*xc+xc.^2+(y+(-1).*yc).^2).^(1/2)+ ...
  mx.*(x.^2+(-2).*x.*xc+xc.^2+(y+(-1).*yc).^2).*(y+(-1).*yc).*sin(atan(x+( ...
  -1).*xc,y+(-1).*yc))));
